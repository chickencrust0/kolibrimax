    ...
    *,
    ...
    try:
        if present:
            ...
            # (POST visit/delete с телом [id]), а НЕ списание занятия.
            #
            # Раньше здесь вызывался burn_one, и повторное нажатие
            # вместо снятия отметки помечало ученику пропуск — ровно
            # противоположное тому, чего ждёт преподаватель. Списание за
            # неявку живёт отдельно, в кнопке «❌» (см. _toggle_absence).
        ...
        # смотреть надо на время внутри target (его печатает
        # impulse_client) и на запись ученика на занятие.
        try:
            schedule_id = str(lesson_id).partition(":")[0]
            mine = await impulse.find_reservations(date_ts, client_id, schedule_id)
            logger.error(f"   записей на это занятие у клиента {client_id}: {len(mine)}")
        except Exception as diag:
            logger.warning(f"   диагностику выполнить не удалось: {diag}")

        await callback.message.answer(f"❌ {esc(str(e))}", parse_mode="HTML")
        return

    logger.info(
        f"✅ {'Присутствие отмечено' if present else f'Снято отметок: {removed}'} "
        f"в CRM: клиент {client_id}, {lesson_id}"
    )
    ...
    if present:
        text = "✅ Отмечено присутствие"
    elif removed:
        text = "↩️ Отметка снята"
    else:
        # Отметки в CRM не было — снимать было нечего. Молчать нельзя:
        # карточка перерисуется, и преподаватель решит, что снятие
        # прошло, хотя в CRM ничего не изменилось.
        text = "↩️ Отметка о присутсвтии уже проставлена. Если это ошибка - обратитесь к менеджеру, чтобы он удалил посещение в CRM"
    await _reply(callback, text)


def _sched_id(reservation) -> Any:
    sched = reservation.get("schedule")
    return sched.get("id") if isinstance(sched, dict) else sched
...
async def _marked_set(impulse: ImpulseCRMClient, date_ts: int, lesson) -> set:
    ...
    Логика сопоставления visit -> клиент живёт в impulse_client
    (find_visits): в записи visit нет поля client, связь идёт через
    visit.service -> абонемент -> клиент. Держать две копии этого
    правила опасно — на одной из копий уже сломалось снятие отметки.

    Если внутренний API недоступен, возвращаем пустое множество: кнопки
    просто останутся неотмеченными.
    """
    try:
        visits = await impulse.find_visits(date_ts, target_id=lesson.get("group_id"))
    except Exception:
        return set()
    if not visits:
        return set()
...
async def teacher_freeze_menu(
    ...
    # Ученики под карточкой занятия — так же, как в отметке
    # посещаемости. Раньше был лишний шаг «Заморозить это занятие» →
    # список детей: заморозка всегда касается КОНКРЕТНОГО ребёнка, и
    # промежуточная кнопка только создавала впечатление, что морозится
    # занятие целиком, для всей группы.
    try:
        customers = await load_customer_map(impulse)
    except Exception as e:
        logger.warning(f"Не удалось загрузить имена учеников: {e}")
        customers = {}

    await callback.message.answer(
        "❄️ <b>Заморозка занятия</b>\n\n"
        "Выберите ученика под нужным занятием — заморозка касается "
        "только его.",
        parse_mode="HTML",
    )
    for lesson in freezable:
        ...
        # show_date=True: карточки идут сплошным списком, без заголовков
        # дня, и без даты видно только время — понять, какое из занятий
        # морозишь, нельзя.
        card = format_lesson(lesson, role="teacher", show_date=True)
        if not students:
            card += "\n\n<i>В занятии нет учеников — морозить некого.</i>"

        await asyncio.sleep(settings.MAX_SEND_DELAY)
        await safe_call(lambda c=card, l=lesson, st=students: callback.message.answer(
            c,
            parse_mode="HTML",
            reply_markup=(
                teacher_freeze_students_keyboard(l["id"], st) if st else None
            ),
        ))


# Обработчик tfrz_lesson: оставлен для старых сообщений в истории чата:
# промежуточный шаг «Заморозить это занятие» убран, но кнопки под уже
# отправленными карточками остаются нажимаемыми.
...
async def teacher_freeze_students(
    ...
    lesson_id = callback.data.partition(":")[2]
    lesson = await get_lesson_snapshot(lesson_id, impulse, cache, db=db)
    if not lesson or not can_freeze(lesson):
        await callback.answer()
        await callback.message.answer(
    ...
    await callback.answer()
    if not students:
        await callback.message.answer("В этом занятии нет учеников — морозить некого.")
        return
    await callback.message.answer(
...
async def _teacher_freeze_ctx(callback: Callback, db: Database, impulse, cache):
    ...
    # rpartition, а НЕ split: id занятия сам содержит двоеточие
    # ("tfrz_pick:116:2026-09-08:754") — client_id всегда последний.
    rest = callback.data.partition(":")[2]
    lesson_id, _, client_raw = rest.rpartition(":")
    if not lesson_id or not client_raw.isdigit():
        await callback.answer("❌ Не удалось определить ученика.")
