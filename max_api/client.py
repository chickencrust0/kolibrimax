...
class MaxBot:
    ...
    async def answer_callback(
        ...
        *,
        ...
        ТРЕБУЕТ ПРОВЕРКИ: официальная документация POST /answers описывает
        только поле `message` (обновление текущего сообщения). Отдельного
        поля для одноразового всплывающего уведомления (аналог
        callback.answer(text, show_alert=True) в Telegram) в описании тела
        запроса не приведено, хотя во вступлении к методу оно упомянуто.
        Пока это не подтверждено — здесь текст, если передан, отправляется
        обычным сообщением пользователю отдельным вызовом на уровне
        диспетчера, а не через это поле.
        """
        body: Dict[str, Any] = {}
