...
class ImpulseCRMClient:
    ...
    async def _request(
        ...
        *,
        ...
        json_body: Optional[Any] = None,   # объект или JSON-массив (см. delete_many)
        attempts: int = 3,
    ) -> Any:
    ...
    async def load(self, entity: str, record_id: Any) -> Optional[Dict[str, Any]]:
        ...
        # Раньше здесь стоял `return items[0]` — «раз что-то пришло,
        # значит это оно». Если сервер игнорирует параметр id и отдаёт
        # список, бот молча брал ПЕРВОГО клиента базы и показывал его
        # данные как чужие: остаток заморозок при этом всегда выглядел
        # нулём, потому что смотрели не в ту карточку.
        if items:
            logger.warning(
                f"⚠️ {entity}/load вернул записи, но ни одна не совпала с "
                f"id={record_id!r} (пришли id: "
                f"{[i.get('id') for i in items[:5]]}). Ищу в общем списке."
            )
        return await self._scan_for(entity, record_id)
    ...
    async def delete(self, entity: str, record_id: Any) -> Dict[str, Any]:
        ...
        Тело — JSON-МАССИВ идентификаторов, а не объект {"id": …}. Так
        описано в справочнике («чаще всего это JSON-массив числовых
        идентификаторов») и так шлёт сам веб-интерфейс:

            POST /api/public/visit/delete
            [478]

        Прежний вариант {"id": 478} молча не удалял ничего.
        """
        return await self.delete_many(entity, [record_id])

    async def delete_many(self, entity: str, record_ids: List[Any]) -> Dict[str, Any]:
        ids = []
        for rid in record_ids:
            try:
                ...
                ids.append(rid)
        return await self._request("POST", entity, "delete", json_body=ids)
    ...
    async def find_visits(
        ...
        """
        Отметки посещения за дату — через внутренний check_visits/visits.

        Публичный visit/list здесь НЕ годится, и дело не в доступе:
        в записи visit вообще НЕТ поля client. Связь с учеником идёт
        через visit.service -> id абонемента (groupAccount), а уже
        абонемент ссылается на клиента. Фильтр по v["client"] не
        совпадал никогда, и снятие отметки отвечало «отметки в CRM нет»
        сразу после успешной отметки.

        Это тот же источник и та же логика сопоставления, которой
        карточка занятия определяет, кто уже отмечен, — а она работает.
        """
        try:
            raw = await self.get_visits(date_ts)
        except ImpulseCRMError as e:
            logger.warning(f"Не удалось получить check_visits/visits: {e}")
            return []
        ...
        client_by_account: Dict[Any, Any] = {}
        if client_id is not None:
            try:
                accounts_by_client = await self.get_accounts_by_client()
            except ImpulseCRMError as e:
                logger.warning(f"Не удалось сопоставить абонементы клиентам: {e}")
                return []
            client_by_account = {
                a.get("id"): cid
        ...
        for v in items:
            ...
            if target_id is not None:
                ...
                if isinstance(v_target, dict) and v_target.get("id") is not None:
                    if not _same_id(v_target["id"], target_id):
                        continue

            if minutes is not None and v.get("minutes") not in (None, minutes):
                continue
            ...
            if client_id is not None:
                ...
                if cid is None:
                    ...
                    if cid is None and isinstance(v.get("client"), dict):
                        cid = v["client"].get("id")
                if cid is None or not _same_id(cid, client_id):
                    continue

            result.append(v)
        return result

    async def uncheck_visit(
        self,
        client_id: Any,
        target: Dict[str, Any],
        date_ts: int,
        ...
        """
        Снять отметку присутствия — удалить запись о посещении.

        Возвращает число удалённых отметок. Ноль означает, что отметки в
        CRM и не было: снимать нечего, и это не ошибка.

        Раньше повторное нажатие вызывало burn_one, то есть вместо снятия
        отметки СПИСЫВАЛО занятие как пропуск — прямо противоположное
        тому, чего ждёт преподаватель.
        """
        minutes = target.get("minutesBegin")
        visits = await self.find_visits(
            date_ts, client_id=client_id, target_id=target.get("id"), minutes=minutes
        )
        if not visits:
            logger.info(
                f"↩️ Снятие отметки: посещений клиента {client_id} на цели "
                f"{target.get('id')} за {date_ts} (minutes={minutes}) не найдено"
            )
            return 0

        ids = [v.get("id") for v in visits if v.get("id") is not None]
        if not ids:
            logger.warning(
                f"⚠️ Посещения найдены, но без id: "
                f"{json.dumps(visits[:2], ensure_ascii=False)[:400]}"
            )
            return 0

        logger.info(f"→ visit/delete: {ids}")
        response = await self.delete_many("visit", ids)
        logger.info(f"← visit/delete: {str(response)[:300]}")
        self._assert_ok(response, "Снятие отметки посещения")
        return len(ids)
    ...
    def _schedule_target(raw: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Берём первый непустой объект из target/group и отдаём его
        ЦЕЛИКОМ — фронтенд кладёт в запрос полный объект, а не ссылку
        {id}. Для индивидуальных занятий целью выступает клиент.

        ВАЖНО: в объект дописываются minutesBegin, minutesEnd и hall
        САМОГО ЗАНЯТИЯ. Это видно по настоящему запросу, который шлёт
        веб-интерфейс CRM при отметке посещения:

            {"client_id": 754,
             "target": {…группа…, "id": 33, "entity": "group",
                        "minutesBegin": 1140, "minutesEnd": 1200,
                        "hall": {…}},
             "date": 1788739200}

        Именно так сервер понимает, КАКОЕ занятие группы отмечают: время
        лежит внутри target, а не отдельным полем рядом. Мы раньше слали
        группу без времени — и получали «отметьте занятие через раздел
        "Отметка посещений" по расписанию».
        """
        target: Dict[str, Any] = {}
        for field in (
        ...
        if minutes_begin is not None:
            target["minutesBegin"] = minutes_begin
        if minutes_end is not None:
            target["minutesEnd"] = minutes_end
        hall = raw.get("hall")
    ...
    async def get_last_accounts(self, client_id: Any) -> Any:
        ...
        return await self._internal_request(
            ...
            json_body={
                # Тот же объект-ссылка, что и везде. С голым числом
                # эндпоинт отвечал HTTP 500: "The identifier id is
                # missing for a query of App\Entity\Client\Client".
                "client": self._ref("client", client_id),
                "clientId": client_id,
                "client_id": client_id,
            },
        )
    ...
    async def find_reservations(
        self, date_ts: int, client_id: Any = None, schedule_id: Any = None
    ) -> List[Dict[str, Any]]:
        """
        Записи клиентов на занятие («запись на занятие» в интерфейсе).

        Именно на этих записях держится раздел «Отметка посещений»: в
        снятом дампе видно, что reservation связывает client, schedule,
        target, date и minutes, и у неё есть поле burned.

        Нужно для диагностики отказа отметки: если у ученика на это
        занятие записи нет, CRM и не сможет отметить посещение — и
        сообщение «отметьте через раздел Отметка посещений» означает
        буквально «сначала запишите ученика».
        """
        try:
            items = await self.list_("reservation")
        except ImpulseCRMError as e:
            logger.warning(f"Не удалось получить reservation/list: {e}")
            return []

        result = []
        for r in items:
            if r.get("date") != date_ts:
                continue
            if client_id is not None and not _same_id(
                _ref_id(r.get("client")), client_id
            ):
                continue
            if schedule_id is not None and not _same_id(
                _ref_id(r.get("schedule")), schedule_id
            ):
                continue
            result.append(r)
        return result
    ...
    @staticmethod
    def _ref(entity: str, record_id: Any) -> Dict[str, Any]:
        """
        Ссылка на сущность в том виде, в каком её ждёт эта CRM.

        Во всех ответах API связанные записи выглядят как объект с id и
        entity — см. reservation, где client, schedule, group и target
        именно такие. Тот же формат сервер ждёт и на входе.
        """
        try:
            record_id = int(record_id)
        except (TypeError, ValueError):
            pass
        return {"id": record_id, "entity": entity}
    ...
    def _assert_ok(response: Any, what: str) -> Any:
        ...
        """
        # Отказ приходит в разных обёртках: где-то success=false, где-то
        # только текстовое сообщение при HTTP 200. Раньше вторая форма
        # проскакивала как успех, и бот рапортовал «отмечено» на отказ
        # вроде «отметьте занятие через раздел отметки посещений».
        if isinstance(response, str) and response.strip():
            raise ImpulseCRMError(f"{what}: {response.strip()[:300]}")
        ...
        if isinstance(response, dict):
            ...
            if text and not any(
                response.get(k) is True for k in ("success", "result", "ok")
            ):
                raise ImpulseCRMError(f"{what}: {text[:300]}")
    ...
    async def check_visit(
        ...
        # ТЕЛО ЗАПРОСА — РОВНО ТАКОЕ ЖЕ, КАК У ВЕБ-ИНТЕРФЕЙСА CRM.
        #
        # Снятый из браузера настоящий запрос отметки посещения:
        #
        #   {"client_id": 754,
        #    "target": {…группа…, "id": 33, "entity": "group",
        #               "minutesBegin": 1140, "minutesEnd": 1200,
        ...
        #    "date": 1788739200}
        #
        # Три поля. Ни account, ни schedule, ни targetValues, ни minutes
        # на верхнем уровне — всё это мы досылали, восстанавливая формат
        # по догадкам.
        #
        # Ключевым, судя по всему, было именно ЛИШНЕЕ поле account:
        # передавая абонемент, мы просили отметить посещение «по
        # абонементу», а сервер отвечал, что для занятия из расписания
        # так нельзя — «отметьте занятие через раздел "Отметка
        # посещений" по расписанию». Абонемент сервер выбирает сам по
        # клиенту и занятию.
        #
        # client_id — обычное число, а не объект-ссылка: так его шлёт
        # фронтенд. Время занятия лежит ВНУТРИ target (см.
        # _schedule_target), а не рядом с ним.
        ...
        #
        # Эндпоинты check_visits вендором не документированы: их форма
        # восстановлена по фронтенду. Когда CRM отвечает отказом вроде
        # «отметьте занятие через раздел отметки посещений», причина
        # всегда в конкретном поле payload — и без полного тела в логе
        # понять, в каком именно, невозможно. Объём тела заметный,
        # поэтому уровень DEBUG: включается через LOG_LEVEL=DEBUG.
        logger.info(
            f"→ check_visits/check: клиент {client_id}, цель "
            f"{target.get('id')} ({target.get('entity')}), "
            f"minutesBegin {target.get('minutesBegin')}, "
            f"minutesEnd {target.get('minutesEnd')}, "
            f"зал {(target.get('hall') or {}).get('id')}, дата {date_ts} "
            f"({datetime.fromtimestamp(date_ts, tz=timezone.utc)} UTC)"
        )
        if target.get("minutesBegin") is None:
            # Без времени внутри target сервер не поймёт, какое занятие
            # группы отмечают, — это ровно тот случай, который раньше
            # заканчивался отказом.
            logger.warning(
                f"⚠️ В target нет minutesBegin — CRM, скорее всего, откажет. "
                f"target={json.dumps(target, ensure_ascii=False)[:300]}"
            )
        ...
        logger.info(f"← check_visits/check: {str(response)[:1000]}")
    ...
    async def burn_visit(
        ...
        payload: Dict[str, Any] = {
            # Клиент передаётся ОБЪЕКТОМ, а не числом.
            #
            # Так работает вся эта CRM: account и target всегда были
            # объектами {id, entity}, и только клиент почему-то уходил
            # голым числом. Что число не годится, доказал ответ соседнего
            # внутреннего эндпоинта:
            #
            #   HTTP 500 client/last_accounts:
            #   "The identifier id is missing for a query of
            #    App\Entity\Client\Client"  (Doctrine\ORM\ORMException)
            #
            # То есть слой Doctrine не может собрать сущность клиента из
            # того, что мы прислали. В check это, по-видимому, приводило
            # не к 500, а к общему отказу «отметьте занятие через раздел
            # Отметка посещений».
            #
            # Числовые формы оставлены рядом: лишнее поле безвредно, а
            # какая из форм нужна серверу — вендором не описано.
            "client": self._ref("client", client_id),
            "client_id": client_id,
            "clientId": client_id,
            "account": account,
    ...
    # Ключи, под которыми может лежать сам текст адреса, если элементы
    # массива — объекты, а не строки. Точный состав элемента вендор не
    # публикует, поэтому проверяются все правдоподобные варианты.
    _ADDRESS_TEXT_KEYS = ("value", "address", "text", "name", "title", "street")

    # Служебные ключи, которые НЕЛЬЗЯ принимать за счётчик, даже если в
    # них лежит число. Без этого списка id=3 внутри объекта адреса
    # выглядел бы как «три заморозки».
    _ADDRESS_SKIP_KEYS = frozenset({
        "id", "type", "typeid", "kind", "branch", "branchid", "client",
        "clientid", "created", "updated", "sort", "order", "position",
        "index", "num", "number", "flat", "house", "building", "korpus",
        "zip", "postcode", "postalcode", "region", "regionid", "city",
        "cityid", "countryid", "country", "lat", "lon", "latitude",
        "longitude",
    })

    @classmethod
    def _address_parts(cls, raw: Any) -> List[str]:
        """
        Поле address приводится к плоскому списку строк-кандидатов.

        ВАЖНО: по документации impulseCRM `address` — это **array**
        («Массив адресов»), а не строка. Раньше здесь стоял str(raw), и
        значение ["3"] превращалось в строку "['3']", которая числом не
        распознавалась — из-за этого счётчик заморозок, вписанный в CRM,
        бот показывал как 0.

        Точная структура элемента вендором не описана: это может быть
        строка, а может быть объект с неизвестным набором ключей. Поэтому
        обход РЕКУРСИВНЫЙ — берутся все строковые значения на любой
        глубине, кроме служебных ключей (id, house, zip и т.п.), в
        которых число означает совсем не заморозки.
        """
        parts: List[str] = []
        ...
        def walk(node: Any, key: str = "") -> None:
            ...
            if isinstance(node, bool):
                return
            if key.replace("_", "").lower() in cls._ADDRESS_SKIP_KEYS:
                return
            text = str(node).strip()
            if text:
                parts.append(text)
    ...
    @staticmethod
    def _as_count(text: str) -> Optional[int]:
        """Число заморозок, если строка состоит ТОЛЬКО из цифр."""
        cleaned = text.strip()
        if cleaned.isdigit():
            try:
    ...
    @classmethod
    def resolve_freezes_field(cls, client: Dict[str, Any]) -> str:
        """
        Поле, в котором у ЭТОГО клиента реально лежит число заморозок.

        Сначала пробуется настроенное (IMPULSE_FIELD_CLIENT_FREEZES),
        затем — кандидаты из IMPULSE_FIELD_CLIENT_FREEZES_ALT.

        Зачем перебор, если имя поля известно: счётчик переезжал с email
        на address, а .env живёт на сервере отдельно от кода и архивом
        не перезаписывается. Настройка со старым именем указывает на
        пустое поле, и бот честно показывает 0 — при том что число в CRM
        стоит. Перебор делает эту рассинхронизацию невозможной, а в лог
        пишется, что настройка устарела.
        """
        configured = cls.freezes_field(client)
        if cls._as_count_in(client.get(configured)) is not None:
            return configured

        for name in settings.IMPULSE_FIELD_CLIENT_FREEZES_ALT:
            if name == configured or name not in client:
                continue
            if cls._as_count_in(client.get(name)) is not None:
                logger.warning(
                    f"⚠️ Счётчик заморозок найден в поле {name!r}, а настроено "
                    f"{configured!r} (в нём числа нет). Читаю из {name!r}. "
                    f"Поправьте IMPULSE_FIELD_CLIENT_FREEZES={name} в .env."
                )
                return name
        return configured

    @classmethod
    def _as_count_in(cls, raw: Any) -> Optional[int]:
        """Число заморозок внутри значения поля, или None."""
        for part in cls._address_parts(raw):
            count = cls._as_count(part)
    ...
    def parse_free_freezes(cls, client: Dict[str, Any]) -> int:
        field = cls.resolve_freezes_field(client)

        if field not in client:
            # Поле вообще не пришло в записи: либо оно называется иначе,
            # либо list не отдаёт его без явного fields. Молчать нельзя —
            # снаружи это неотличимо от честного нуля.
            logger.warning(
                f"⚠️ В записи клиента {client.get('id')} нет поля {field!r}. "
                f"Есть поля: {', '.join(sorted(client.keys()))[:400]}. "
                f"Запустите freezes_probe.py, чтобы увидеть структуру."
            )
            return 0

        raw = client.get(field)
        parts = cls._address_parts(raw)
        if not parts:
            return 0

        # Значением считается ТОЛЬКО целиком числовой элемент.
        #
        # Выбирать цифры из строки нельзя: адрес часто заполнен
        # по-настоящему, и «ул. Ленина, д. 5» превратилось бы в пять
        # беспричинных заморозок. Поэтому в массиве ищется отдельный
        # элемент-число — рядом с ним может спокойно лежать настоящий
        # адрес, они друг другу не мешают.
        for part in parts:
            count = cls._as_count(part)
            if count is not None:
                logger.debug(
                    f"❄️ Клиент {client.get('id')}: заморозок {count} "
                    f"(поле {field} = {raw!r})"
                )
                return count

        logger.warning(
            f"⚠️ В поле {field} клиента {client.get('id')} нет числового "
            f"элемента: {raw!r} (разобрано как {parts!r}) — считаю 0 "
            f"заморозок. Число должно быть ОТДЕЛЬНОЙ строкой адреса, а не "
            f"дописано к существующей."
        )
        return 0
    ...
    async def get_free_freezes(self, client_id: Any) -> int:
        ...
        if not client:
            # Третья возможная причина нуля, до сих пор молчаливая:
            # карточку клиента вообще не нашли. Считать это «нулём
            # заморозок» — значит показать родителю неправду.
            raise ImpulseCRMError(
                f"Карточка клиента {client_id} не найдена в CRM — "
                f"остаток заморозок посмотреть не по чему"
            )
        return self.parse_free_freezes(client)
    ...
    async def set_free_freezes(self, client_id: Any, value: int) -> int:
        ...
        # То же поле, из которого читаем: иначе списание ушло бы в одно
        # место, а показ брался бы из другого.
        field = self.resolve_freezes_field(client)
        raw = client.get(field)

        # Форма поля сохраняется: address — массив, и подсунуть туда
        # строку значит либо получить отказ CRM, либо испортить запись.
        # Заменяется ТОЛЬКО числовой элемент — настоящий адрес, если он
        # лежит рядом в том же массиве, остаётся нетронутым.
        if isinstance(raw, (list, tuple)) or raw is None:
            # Исходные элементы переносятся КАК ЕСТЬ, а не через
            # _address_parts: если адрес хранится объектами, разбор в
            # плоские строки потерял бы структуру и затёр настоящий
            # адрес. Меняется ровно один элемент — числовой.
            items = list(raw or [])
            replaced = False
            new_items: List[Any] = []
            for item in items:
                if (
                    ...
                    and self._as_count(str(item)) is not None
                ):
                    new_items.append(str(value))
                    replaced = True
                else:
                    new_items.append(item)
            if not replaced:
                new_items.append(str(value))
            new_value: Any = new_items
        else:
            new_value = str(value)
        ...
        logger.info(
            f"✅ Остаток беспричинных заморозок клиента {client_id}: {value} "
            f"(поле {field} = {new_value!r})"
        )
        return value
